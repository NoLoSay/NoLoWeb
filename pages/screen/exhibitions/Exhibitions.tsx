...
const styles: { [key: string]: string } = {
  ...
  divBlockExhibitionInfos:
    "flex-1 w-full " + "sm:items-start pl-5 " + "md:pl-5 sm:pl-0",
  ...
  exhibitionDescription:
    "flex-[0_auto] pr-0 text-justify w-full " + "sm:text-start",
  ...
  const handleDeleteExhibition = async (exhibitionId: any) => {
    try {
      const response = await fetch(
        `http://localhost:3001/exhibitions/${exhibitionId}`,
        {
          method: "DELETE",
          headers: {
      ...
      if (response.ok) {
        const updatedExhibitions = exhibitions.filter(
          (exhibition: any) => exhibition.id !== exhibitionId
        );
        setExhibitions(updatedExhibitions);
        console.log("Exhibition deleted successfully");
      } else {
        throw new Error(
          `HTTP status ${response.status}: Failed to delete the exhibition`
        );
      }
    } catch (error) {
    ...
    switch (buttonName) {
      case "handleGoToArtwork":
        try {
          ...
          navigate("/places/exhibitions/artworks", {
            state: { item: artworks, exhibitionId: exhibitionId },
          });
        } catch (error) {
  ...
  if (!exhibitions || exhibitions.length === 0) {
    return (
      <Fragment>
        <div className={`divBlockTitlePage ${styles["divBlockTitlePage"]}`}>
          ...
          <div className={`divTitlePage ${styles["divTitlePage"]}`}>
            <h1 className={`pageTitle ${styles["pageTitle"]}`}>
              {textData.page.screen.exhibitions.myExhibition}
            </h1>
          </div>
        ...
        <div className={`noExhibitions ${styles["noExhibitions"]}`}>
          {textData.page.screen.exhibitions.noAvaible}
        </div>
      </Fragment>
    );
  } else
      ...
      <Fragment>
        <section className={`exhibitionsPage ${styles["exhibitionsPage"]}`}>
          <div className={`divBlockTitlePage ${styles["divBlockTitlePage"]}`}>
            ...
            <ButtonBase
              disableRipple
              onClick={() => handleAddExhibition(siteId)}
            >
              <div className={styles.buttons}>
                {textData.page.screen.exhibitions.addExhibition}
              </div>
          ...
          <div
            className={`divBlockExhibitionList ${styles["divBlockExhibitionList"]}`}
          >
            <div className={`divExhibitionList ${styles["divExhibitionList"]}`}>
              {exhibitions.map((exhibition: any) => (
                ...
                <div
                  key={exhibition.id}
                  className={`divExhibition ${styles["divExhibition"]}`}
                >
                  <img
                    src=""
                    loading="lazy"
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        className={`divModifyButtons ${styles["divModifyButtons"]}`}
                      >
                        <ButtonBase
                          disableRipple
                          onClick={() =>
                    ...
                    >
                      ...
                      >
                        <div
                          className={`divGeneralInformation ${styles["divGeneralInformation"]}`}
                        >
                          <div className={`placeText ${styles["placeText"]}`}>
                            {exhibition.site.address.street},{" "}
                            {exhibition.site.address.zip}
                            {exhibition.site.address.City
                              ? `, ${exhibition.site.address.City.name}`
                              : ""}
                            {exhibition.site.address.City &&
                            exhibition.site.address.City.Department
                              ? `, ${exhibition.site.address.City.Department.name}`
                              : ""}
                            {exhibition.site.address.City &&
                            exhibition.site.address.City.Department &&
                            exhibition.site.address.City.Department.Country
                              ? `, ${exhibition.site.address.City.Department.Country.name}`
                              : ""}
                          </div>
                          <div className={`dateText ${styles["dateText"]}`}>
                            {textData.page.screen.exhibitions.from}{" "}
                            {new Date(
                              exhibition.startDate
                            ).toLocaleDateString()}{" "}
                            <br /> {textData.page.screen.exhibitions.to}{" "}
                            {new Date(exhibition.endDate).toLocaleDateString()}
                          </div>
                          <div className={`priceText ${styles["priceText"]}`}>
                            {exhibition.site.price}{" "}
                            {textData.page.screen.exhibitions.euro}
                          </div>
                          <div
                            className={`artworkNbText ${styles["artworkNbText"]}`}
                          >
                            {" "}
                            {textData.page.screen.exhibitions.nbArtwork}
                          </div>
                        </div>
                        <div
                          className={`exhibitionDescription ${styles["exhibitionDescription"]}`}
                        >
