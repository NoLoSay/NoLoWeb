...
const styles: { [key: string]: string } = {
  ...
  container_7: "flex flex-row items-center gap-8 text-gray-200 bg-transparent hover:underline hover:cursor-pointer",
  container_8: "font-medium text-base",
  container_9: "rounded-full bg-gray-300 hover:cursor-pointer flex items-center justify-center py-2 px-6 gap-2 text-base-white font-semibold hover:underline",
  container_10: "font-semibold text-base",
  container_11: "p-3",
  container_12: "flex flex-row items-center px-10 h-24 shadow-md",
  container_13: "flex flex-row items-center gap-8 text-gray-200 bg-transparent hover:underline hover:cursor-pointer",
  container_14: "rounded-full bg-gray-300 hover:cursor-pointer flex items-center justify-center py-2 px-6 gap-2 text-base-white font-semibold hover:underline",
...
const LogoButton = ({ handleChangePage }: logoButtonProps) => {
  return (
    <div className={`container_0 ${styles.container_0}`}>
      <img className={`container_1 ${styles.container_1}`} alt="" src="/images/logo/nologo.png" />
      <button
        className={`container_2 ${styles.container_2}`}
        onClick={() => handleChangePage("/home")}
      >
        <img className={`container_3 ${styles.container_3}`} alt="" src="/images/logo/nolosay-black.png" />
      </button>
    </div>
  );
};
...
const LoginButton = ({ handleChangePage }: loginButtonProps) => {
  return (
    <div className={`container_6 ${styles.container_6}`}>
      ...
      >
        <div className={`container_10 ${styles.container_10}`}>{`Inscription`}</div>
      </button>
    </div>
  );
};
