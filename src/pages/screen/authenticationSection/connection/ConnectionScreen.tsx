...
const styles: { [key: string]: string } = {
  ...
  form: 'flex flex-1 flex-col gap-5 items-center relative text-center w-2/5 ' + 'sm:w-3/4 ' + 'md:w-3/5',

  formInput: 'bg-gray-50 w-auto font-normal text-xl p-2 relative rounded-1.5lg w-full',
  ...
  otherConnectionsDivSeperationDiv: 'flex flex-row items-center justify-between w-1/2',

  otherConnectionsDivSeperationDivLine: 'bg-black w-1/3 h-px',

  otherConnectionsDivSeperationDivText: 'font-light font-poppins text-black text-xs',

  otherConnectionsDivButtonDiv: 'flex flex-row items-center justify-center gap-5 relative w-1/2',

  otherConnectionsDivButtonDivButtons: 'hover:cursor-pointer',

  noAccountText: 'font-normal font-poppins relative text-center text-gray-300 text-xs',

  noAccountButton: 'font-bold font-poppins bg-transparent hover:underline hover:cursor-pointer text-black',
}

export const ConnectionScreen = (): JSX.Element => {
  const navigate = useNavigate()
  const { setUsername, setPassword, showPassword, setShowPassword, connect, error } = ConnectionController({ navigate })
  ...
  return (
    <div className={`ConnectionScreen/mainDiv ${styles['mainDiv']}`}>
      <div className={`ConnectionScreen/titleDiv ${styles['titleDiv']}`}>
        ...
        />
        <div className={`ConnectionScreen/titleDivTextDiv ${styles['titleDivTextDiv']}`}>
          <p className={`ConnectionScreen/titleDivTextDivTitle ${styles['titleDivTextDivTitle']}`}>
            {textData.page.screen.authentificationSection.connection.connecty}
          </p>
          <p className={`ConnectionScreen/titleDivTextDivSubtitle ${styles['titleDivTextDivSubtitle']}`}>
      ...
      >
        ...
        />
        <div className={`ConnectionScreen/formPasswordDiv ${styles['formPasswordDiv']}`}>
          <input
            className={`ConnectionScreen/formInput ${styles['formInput']}`}
        ...
        <button className={`ConnectionScreen/formConnectionButton ${styles['formConnectionButton']}`}>
          {textData.page.screen.authentificationSection.connection.connectme}
        </button>
      </form>
      {error && <p className={`ConnectionScreen/errorMessage ${styles['errorMessage']}`}>{error}</p>}
      <div className={`ConnectionScreen/otherConnectionsDiv ${styles['otherConnectionsDiv']}`}>
        <div
        ...
        <div className={`ConnectionScreen/otherConnectionsDivButtonDiv ${styles['otherConnectionsDivButtonDiv']}`}>
          <img
      ...
      <p className={`ConnectionScreen/noAccountText ${styles['noAccountText']}`}>
