...
const styles: { [key: string]: string } = {
  mainDiv: 'bg-white flex flex-col gap-10 items-center justify-center mt-8 relative w-full',
  smMainDiv: 'sm:gap-0',
  mdMainDiv: 'md:gap-5',
  ...
  titleDivFilterDiv: 'absolute bg-gray-300 bg-opacity-65 h-full rounded-2.5xl w-full',

  titleDivText: 'font-black font-helvetica py-5 relative text-center text-white text-6xl',
  ...
  return (
    ...
    >
      ...
      <img
        ...
        alt='Vector'
      />
      <div className={`ArtworkToTranslateSelectionScreen/titleDiv ${styles['titleDiv']}`}>
        <div className={`ArtworkToTranslateSelectionScreen/titleDivFilterDiv ${styles['titleDivFilterDiv']}`} />
        <p
          className={`ArtworkToTranslateSelectionScreen/titleDivText ${styles['titleDivText']} ${styles['smTitleDivText']} ${styles['mdTitleDivText']}`}
        >
          {
            textData.page.screen.creationSection.artworkToTranslateSelectionSection.artworkToTranslateSelectionSection
              .totranslate
          }
        </p>
      </div>
      ...
      >
        {artworkList.map(({ title, imageSrc, imageAlt, description }, index) => (
          <ArtworkCard
            key={index}
            title={title}
            imageSrc={imageSrc}
