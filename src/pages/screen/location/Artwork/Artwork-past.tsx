...
const styles = {
  ...
  divButtonDontSave: "bg-red-500 py-2 px-4 rounded hover:bg-red-700 cursor-pointer",
  divButtonSave: "bg-green-500 py-2 px-4 rounded hover:bg-green-700 cursor-pointer",
...
const ArtworkModificationPage = () => {
  ...
  const fetchAllArtworks = async () => {
    try {
      const response = await fetch('https://api.nolo.aurelenc.com/items', {
        headers: {
    ...
    const handleDeleteArtwork = async (artworkId: any, exhibitionId?: any) => {
      try {
        ...
        const linkDeleteResponse = await fetch(`https://api.nolo.aurelenc.com/exhibitions/${exhibitionId}/items/${artworkId}`,
          {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${user.accessToken}`,
