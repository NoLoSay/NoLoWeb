...
function NewsletterField() {
  return (
    <div className={`flex flex-col w-full bg-sky-50 bg-center bg-cover items-center py-10 space-y-5`}>
      <p className={`container_0 ${styles.container_0}`}>{textData.page.components.newsletter.subscribeNews}</p>
      <div className={`container_1 ${styles.container_1}`}>
        <TextField
          id='outlined-basic'
          label='Addresse mail'
        />
        <Button sx={{ backgroundColor: '#f5cb42', color: '#000' }}>{textData.page.components.newsletter.sub}</Button>
      </div>
    </div>
  )
}
